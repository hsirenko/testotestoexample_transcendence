...
const last12 = (): string[] => {
    ...
    return Array.from({ length: 12 }, (_, i) =>
        new Date(n.getFullYear(), n.getMonth() - 11 + i).toLocaleString('en-US', {
            month: 'short',
            year: 'numeric',
        })
    );
...
async function drawLifePie(url: string) {
    ...
    try {
        ...
        new Chart(document.getElementById('fs-life-chart') as HTMLCanvasElement, {
            ...
            data: {
                labels: ['Wins', 'Losses'],
                datasets: [{ data: [wins, losses], backgroundColor: ['#4ade80', '#f87171'] }],
            },
    ...
    try {
        ...
        new Chart(document.getElementById('fs-goals-chart') as HTMLCanvasElement, {
            ...
            data: {
                ...
                datasets: [{ data: [scored, conceded], backgroundColor: ['#38bdf8', '#f87171'] }],
            },
            options: { plugins: { legend: { labels: { color: '#fff', boxWidth: 10 } } } },
        });
    } catch (err) {
        ...
        new Chart(document.getElementById('fs-hits-chart') as HTMLCanvasElement, {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: 'Scored',
                        data: labels.map((l) => s[l.slice(0, 3)] ?? 0),
                        backgroundColor: 'rgba(34,211,238,0.9)',
                        borderRadius: 4,
                    },
                    {
                        label: 'Conceded',
                        data: labels.map((l) => c[l.slice(0, 3)] ?? 0),
...
async function drawTrophies(url: string): Promise<void> {
    ...
    try {
        ...
        container.innerHTML = `<p class="py-4 text-center text-red-400">Failed to load stats.</p>`;
    }
}
...
function closeFriendStats(): void {
    ...
    ['fs-monthly-chart', 'fs-life-chart', 'fs-goals-chart', 'fs-hits-chart'].forEach(destroy);
}
...
export function openFriendStats(
    ...
    //header visuals
    (document.getElementById('friendstats-avatar') as HTMLImageElement).src = resolveAvatar(
        friend.avatar_url
    );
...
//close button
document.getElementById('friendstats-close')?.addEventListener('click', closeFriendStats);

//expose for friends.ts
declare global {
    interface Window {
        openFriendStats?: typeof openFriendStats;
    }
}
