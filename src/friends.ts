...
function render(friends: any[]): void {
    ...
    friends.forEach((f) => {
        ...
        yesBtn.addEventListener('click', async () => {
            ...
            try {
                const r = await fetch(`/api/users/remove-friend/${friendId}`, {
                    method: 'DELETE',
                    headers: getAuthHeader(),
                });
...
export async function loadFriendsSidebar(): Promise<void> {
    ...
    try {
        ...
        list.innerHTML = `<p class="text-red-400">${e?.error ?? 'Failed to load friends.'}</p>`;
    }
}
...
export function initFriendsSidebarToggle(): void {
    const sidebar = document.getElementById('friends-sidebar') as HTMLElement | null;
