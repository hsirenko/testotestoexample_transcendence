...
function deleteAvatarFile(rel?: string | null): void {
    ...
    fs.promises.unlink(full).catch(() => console.warn('⚠️  could not delete old avatar:', full));
}
...
export default async function userRoutes(fastify: FastifyInstance) {
    fastify.put(
        ...
        async (req: FastifyRequest, reply: FastifyReply) => {
            ...
            if (username && username.length < 2) {
                return reply
                    .status(400)
                    .send({ error: 'Username must be at least 2 characters long' });
            }
            ...
            if (newPassword) {
                if (!oldPassword) {
                    return reply
                        .status(400)
                        .send({ error: 'Old password is required to change your password' });
                }
    ...
    fastify.get('/api/users/:id', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    fastify.delete('/api/users/avatar', { preHandler: authMiddleware }, async (req, reply) => {
        const { userId } = (req as FastifyRequest & { user: JWTPayload }).user;
    ...
    fastify.put('/api/users/avatar', { preHandler: authMiddleware }, async (req, reply) => {
        const { userId } = (req as FastifyRequest & { user: JWTPayload }).user;

        /* 1) fetch current avatar path once – but DON'T delete yet */
        const prev = db.prepare('SELECT avatar_url FROM users WHERE id = ?').get(userId) as {
            avatar_url?: string | null;
        };
        ...
        if (req.isMultipart()) {
            ...
            db.prepare('UPDATE users SET avatar_url = ? WHERE id = ?').run(relative, userId);
        ...
        if (buf.length > 5 * 1024 * 1024) return reply.code(400).send({ error: 'Image too large' });
