...
export default async function notifRoutes(fastify: FastifyInstance) {
    ...
    fastify.delete('/api/notifications/:id', { preHandler: authMiddleware }, async (req, reply) => {
        ...
        const row = db
            .prepare(`SELECT type, reference_id FROM notifications WHERE id = ? AND user_id = ?`)
            .get(notifId, userId);
        ...
        if (row && row.type === 'challenge' && row.reference_id) {
            ...
            const info = db
                .prepare(
                    `INSERT INTO notifications (user_id, type, text) VALUES (?, 'challenge_declined', ?)`
                )
                .run(row.reference_id, text);
    ...
    fastify.delete('/api/notifications', { preHandler: authMiddleware }, async (req, reply) => {
        const { userId } = (req as any).user as JWTPayload;
        db.prepare(
