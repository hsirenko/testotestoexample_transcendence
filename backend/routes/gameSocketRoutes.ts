...
export default async function gameSocketRoutes(fastify: FastifyInstance) {
    fastify.get('/ws/game', { websocket: true }, (socket: WebSocket, request: FastifyRequest) => {
        ...
        socket.on('close', () => {
            ...
            handleGameResult(currentGame.id, winnerId, loserId, leftScore, rightScore);
            ...
            WHERE id = ?
          `
                ).run(winnerId, leftScore, rightScore, matchId);
          ...
          `
                ...
                const loser = db.prepare(`SELECT trophies FROM users WHERE id = ?`).get(loserId);
