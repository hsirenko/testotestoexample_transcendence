...
export function leaveTournament(code: string, userId: number): void {
  ...
  `
    ...
    const roster = tour.playerIds.map((id) => {
        const { username } = db.prepare('SELECT username FROM users WHERE id = ?').get(id) as {
            username: string;
        };
        return { id, username };
    });
...
export function createTournament(name: string, creatorId: number): LiveTournament {
    ...
    tours.set(code, tour);
    db.prepare(`INSERT INTO tournament_players (tournament_id, user_id) VALUES (?,?)`).run(
        tour.id,
        creatorId
    );
    return tour;
...
export function cancelTournament(code: string): void {
    ...
    /* 3 – remove from DB */
    db.prepare('DELETE FROM tournament_players WHERE tournament_id = ?').run(tour.id);
...
export function handleGameResult(
    ...
    if (tour.finalGame === gameId) {
        ...
        db.prepare(`UPDATE tournaments SET status='finished', winner_id=? WHERE id=?`).run(
            winnerId,
            tour.id
        );
        ...
        recordTournamentScoresOnChain(tour.id).catch((error) => {
            console.error(
                `Failed to initiate blockchain recording for tournament ${tour.id}:`,
                error
            );
        });
    }
...
function isFinished(gameId: string) {
    const row = db.prepare(`SELECT winner_id FROM matches WHERE game_id=?`).get(gameId) as {
