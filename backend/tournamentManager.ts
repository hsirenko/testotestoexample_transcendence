...
const genCode        = customAlphabet(ALPHA, 8);
...
export function createTournament(name: string, creatorId: number): LiveTournament {
  const code = genCode();
  const insert = db.prepare(`INSERT INTO tournaments (name, code, created_by) VALUES (?,?,?)`)
...
export function handleGameResult(gameId: string,
                                 winnerId: number,
                                 loserId:  number,
                                 scoreP1:  number,
                                 scoreP2:  number) {
  ...
  const tour = tours.get(code)!;
  const match = db.prepare(`SELECT id FROM matches WHERE game_id = ?`).get(gameId) as { id:number };
  ...
  if (tour.semiGames.includes(gameId) && !tour.finalGame && tour.semiGames.every(g => isFinished(g))) {
    ...
    const w1 = fetchWinner(g1), w2 = fetchWinner(g2);
  ...
  if (tour.finalGame === gameId) {
    ...
    db.prepare(`UPDATE tournaments SET status='finished', winner_id=? WHERE id=?`)
...
function createGameRowAndInstance(tour: LiveTournament, p1:number, p2:number) {
  const nanoid  = genCode();                // reuse same alphabet/length
  const game    = new Game(nanoid);
  games.set(nanoid, game);
  gameToTourCode.set(nanoid, tour.code);

  db.prepare(`
    INSERT INTO matches (tournament_id, game_id, player1_id, player2_id)
    VALUES (?,?,?,?)`).run(tour.id, nanoid, p1, p2);
...
function isFinished(gameId:string) {
  const row = db.prepare(`SELECT winner_id FROM matches WHERE game_id=?`).get(gameId) as { winner_id:number|null };
  return row.winner_id != null;
}

function fetchWinner(gameId:string) {
  const row = db.prepare(`SELECT winner_id FROM matches WHERE game_id=?`).get(gameId) as { winner_id:number };
